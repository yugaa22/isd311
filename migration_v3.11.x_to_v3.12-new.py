    ...
    try:
        for gate_pipeline_map in gate_pipeline_maps:
            ...
            try:
                ...
                logging.warning(f"Failed to update gate : {url} and the exception is : ", exc_info=True)
    except Exception as e:
        raise e
...
def update_gate(gate_response, pipeline_id, gate_id, cookie):
    try:
        ...
        # let us neither rollback not raise exception here!
        logging.warning("Failed to update the gate data for pipeline_id : \n" + str(pipeline_id) + " gate_id : " + str(
            gate_id) + " gate_response : " + str(gate_response) + '\n')
        # platform_conn.rollback()
        # raise e
